...
class BertModel(object):
  ...
  def __init__(self,
    ...
    if polarity_ids is None:
      polarity_ids = tf.zeros(shape=[batch_size, seq_length], dtype=tf.int32)

    if intensity_ids is None:
      intensity_ids = tf.zeros(shape=[batch_size, seq_length], dtype=tf.int32)
    ...
    with tf.variable_scope(scope, default_name="bert"):
      with tf.variable_scope("embeddings"):
        ...
        self.embedding_output = embedding_postprocessor(
            ...
            polarity_vocab_size=5,
            polarity_embedding_name="polarity_embeddings",
            use_intensity=True,
            intensity_ids=intensity_ids,
            intensity_vocab_size=5,
            intensity_embedding_name="intensity_embeddings",            
            initializer_range=config.initializer_range,
...
def embedding_postprocessor(input_tensor,
  ...
  if use_polarity:
    if polarity_ids is None:
      raise ValueError("`polarity_ids` must be specified if"
  ...
  if use_intensity:
    if intensity_ids is None:
      raise ValueError("`intensity_ids` must be specified if"
                       "`use_intensity` is True.")
