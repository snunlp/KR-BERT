...
def kosac_evaluate(model, data_loader, metrics, device):
    if model.training:
        model.eval()
    ...
    for step, mb in tqdm(enumerate(data_loader), desc='steps', total=len(data_loader)):
        token_b, polarity_b, intensity_b, y_mb = map(lambda elm: elm.to(device), mb)

        with torch.no_grad():
            y_hat_mb = model(token_b, polarity_ids=polarity_b, intensity_ids=intensity_b)
