...
# modified by sut-i (2019-11-09)
# Forward match와 Backward match 중 vocab 내에서의 순위가 높은 것을 선택함.
...
class FullTokenizer(object):
  ...
  def tokenize(self, text):
    ...
    for token in self.basic_tokenizer.tokenize(text):
      t0, i0 = self.wordpiece_tokenizer0.tokenize(token)
...
class WordpieceTokenizer(object):
  ...
  def tokenize(self, text):
    ...
    return output_tokens
    # todo: return 값을 [(token0, id0), (token1, id1), ...]로 바꾸기


class WordpieceBackwardTokenizer(object):
  """Runs WordPiece tokenziation. (modified by sut-i on 2019-11-04)"""
  ...
  def tokenize(self, text):
    ...
    This uses a greedy longest-match-last algorithm to perform tokenization
    using the given vocabulary.
